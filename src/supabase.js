...
export async function syncCalendarLocations(showId,events){
  ...
  const {data:existing,error:loadError}=await supabase.from('production_locations').select('*').eq('show_id',showId);
